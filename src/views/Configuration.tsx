...
import { openProfileModal } from '../store/appSlice';
...
const Configuration: React.FC = () => {
  ...
  const menuItems: { id: ConfigPanel; name: string; icon: string }[] = [
    ...
    { id: 'personalization', name: t('personalization'), icon: 'personalization' },
      ...
      <aside className="w-64 lg:w-80 flex-shrink-0 p-4 pr-0">
        <div className="h-full bg-gray-100/80 dark:bg-gray-800/50 rounded-xl p-4 flex flex-col">
            <button
                ...
                className="flex items-center gap-3 p-2 mb-4 rounded-lg hover:bg-gray-200/80 dark:hover:bg-gray-700/80 transition-colors text-left"
            >
                <div className="w-10 h-10 md:w-12 md:h-12 bg-gray-300 dark:bg-gray-600 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {user.avatarUrl ? (
                        ...
                        <AppIcon name="user" className="w-6 h-6 md:w-8 md:h-8 text-gray-600 dark:text-gray-300"/>
                    )}
                </div>
                <div className="overflow-hidden">
                    <p className="font-semibold text-base md:text-lg text-gray-800 dark:text-gray-100 truncate">{user.name}</p>
                    <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 truncate">{user.email}</p>
                </div>
            </button>

            <div className="relative mb-4">
                <AppIcon name="search" className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500" />
                <input type="text" placeholder={t('find_a_setting')} className="w-full pl-10 pr-4 py-2 rounded-md bg-white dark:bg-gray-700/80 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-[var(--primary-color)] focus:outline-none"/>
            </div>
