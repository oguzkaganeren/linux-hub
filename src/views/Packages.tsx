...
const Packages: React.FC = () => {
    ...
    const [selectedCategory, setSelectedCategory] = useState<Category>(packageData[0]);
    ...
    const displayTitle = debouncedSearchTerm ? `${t('search_results_for')} "${debouncedSearchTerm}"` : selectedCategory.name;
        ...
        <div className="h-full flex">
            ...
            <main className="flex-grow p-4 md:p-6 flex flex-col">
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <h1 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-gray-100">{displayTitle}</h1>
                    <div className="relative w-full md:w-1/3">
                        <AppIcon name="search" className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 z-10" />
                        <input
                            type="text"
                            ...
                            >
                                {filteredApps.map(app => (
                                    <motion.div key={app.pkg} variants={itemVariants}>
                                        <AppCard app={app} onShowDetails={() => setSelectedApp(app)} />
                                    </motion.div>
                                ))}
                            </motion.div>
                        )}
